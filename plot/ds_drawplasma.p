...
set output "svn_plasma_ds.eps"

set size square .66

set title " Delta and Size per Commit - PLASMA"
...
#error(x, dx, y, dy)=(x/y) * sqrt( (dx/x)**2.0 + (dy/y)**2.0 )
   ...
   plot "svn_plasma.txt" using 1:8 title "Size/nTC" with linespoints ls 4 axes x1y1,\
        "" using 1:9 title "nDelta/nTC" with linespoints ls 6 axes x1y2


!epstopdf svn_plasma_ds.eps && rm svn_plasma_ds.eps
