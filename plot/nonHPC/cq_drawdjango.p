...
set output "bb_django_cq.eps"

set size square .66

set title " Commit Quality - DJANGO"
...
#error(x, dx, y, dy)=(x/y) * sqrt( (dx/x)**2.0 + (dy/y)**2.0 )
   ...
   plot "bb_django.txt" using 1:3 title "nTC" with linespoints ls 4 axes x1y1,\
        "" using 1:4 title "nUC" with linespoints ls 5 axes x1y1, \
        "" using 1:5 title "nUC/nTC Ratio" with linespoints ls 6 axes x1y2


!epstopdf bb_django_cq.eps && rm bb_django_cq.eps
