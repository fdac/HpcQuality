...
set output "bb_tutorialbb_ds.eps"

set size square .66

set title " Delta and Size per Commit - Tutorial-BB"
...
#error(x, dx, y, dy)=(x/y) * sqrt( (dx/x)**2.0 + (dy/y)**2.0 )
   ...
   plot "bb_tutorialbb.txt" using 1:8 title "Size/nTC" with linespoints ls 4 axes x1y1,\
        "" using 1:9 title "Delta/nTC" with linespoints ls 6 axes x1y2


!epstopdf bb_tutorialbb_ds.eps && rm bb_tutorialbb_ds.eps
