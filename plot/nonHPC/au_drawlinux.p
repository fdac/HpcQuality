...
set output "linux_au.eps"

set size square .66

set title " Commits per Author - Linux"
...
#error(x, dx, y, dy)=(x/y) * sqrt( (dx/x)**2.0 + (dy/y)**2.0 )
   ...
   plot "linux.txt" using 1:10 title "nUC/nAuthor" with linespoints ls 4 axes x1y1,\
        "" using 1:5 title "nAuthor" with linespoints ls 6 axes x1y2


!epstopdf linux_au.eps && rm linux_au.eps
