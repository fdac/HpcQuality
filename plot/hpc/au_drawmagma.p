...
set output "svn_magma_au.eps"

set size square .66

set title " Commits per Author - MAGMA"
...
#error(x, dx, y, dy)=(x/y) * sqrt( (dx/x)**2.0 + (dy/y)**2.0 )
   ...
   plot "svn_magma.txt" using 1:7 title "nUC/nAuthor" with linespoints ls 4 axes x1y1,\
        "" using 1:6 title "nAuthor" with linespoints ls 6 axes x1y2


!epstopdf svn_magma_au.eps && rm svn_magma_au.eps
