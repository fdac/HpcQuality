...
set output "gh_openSHMEM_cq.eps"

set size square .66

set title " Commit Quality - openSHMEM"
...
set xtics (  "2010" 1, "2011" 2, "2012" 3, "2013" 4, "2014" 5)
...
#error(x, dx, y, dy)=(x/y) * sqrt( (dx/x)**2.0 + (dy/y)**2.0 )
   ...
   plot "gh_openSHMEM.txt" using 1:3 title "nTC" with linespoints ls 4 axes x1y1,\
        "" using 1:4 title "nUC" with linespoints ls 5 axes x1y1, \
        "" using 1:5 title "nUC/nTC Ratio" with linespoints ls 6 axes x1y2


!epstopdf gh_openSHMEM_cq.eps && rm gh_openSHMEM_cq.eps
